...
finance_ex1 = finance_ex1.drop(columns=["证券代码"]).copy()

low_debt_profit_2020 = (
    finance_ex1.loc[
        (finance_ex1["年份"] == 2020)
        & (finance_ex1["净利润_亿元"] > 0)
        & (finance_ex1["资产负债率"] < 0.6),
...
用 `pd.cut()` 根据 `总资产_亿元` 生成 `资产规模`：`0-100` 为“小”，`100-1000` 为“中”，`1000` 以上为“大”。统计 2020 年不同 `资产规模` 的公司数量，赋值给变量 `size_counts_2020`。要求输出计数结果。"""
    ...
    code(
        """finance_ex1["资产规模"] = pd.cut(
    finance_ex1["总资产_亿元"],
    bins=[0, 100, 1000, np.inf],
    labels=["小", "中", "大"],
)

size_counts_2020 = finance_ex1.loc[finance_ex1["年份"] == 2020, "资产规模"].value_counts()
size_counts_2020"""
    ),
    md(
...
从带代码的财务表中取前 12 行，复制为 `dirty_ex1`。人为制造以下问题：把第 2 行 `营业收入_亿元` 改成缺失值；把第 3 行 `净利润_亿元` 改成 `"--"`；把第 4 行 `总资产_亿元` 改成带逗号的字符串；再重复添加第 1 行。完成缺失值检查、重复行检查、去重、数值转换。最终结果赋值给变量 `dirty_ex1_clean`，要求 `总资产_亿元` 和 `净利润_亿元` 都是数值列。"""
    ),
    code(
...
把财务表和公司信息表合并为 `analysis_ex4`。按 `行业名称` 和 `年份` 分组，计算 `公司数`、`营业收入合计_亿元`、`平均净利率`、`平均资产负债率`，赋值给变量 `industry_year_summary`。显示前 12 行。"""
    ...
    code(
        """analysis_ex4 = finance_code.merge(
...
在 2020 年数据中，按 `行业名称` 分组，找出每个行业 `营业收入_亿元` 最高的 2 家公司，赋值给变量 `top2_revenue_by_industry`。结果只保留 `行业名称`、`证券代码`、`证券简称`、`营业收入_亿元`、`净利率`，并按行业名称和营业收入排序。"""
    ...
    analysis_ex4[analysis_ex4["年份"] == 2020]
...
用 `pivot_table()` 把 `analysis_ex4` 整理为每家公司一行、年份为列、值为 `营业收入_亿元` 的宽表，赋值给变量 `revenue_wide_ex5`。计算 `收入增长率_2018_2020 = 2020 / 2018 - 1`，按增长率从高到低排序，显示前 10 行。"""
    ...
    code(
        """revenue_wide_ex5 = analysis_ex4.pivot_table(
    index=["证券代码", "证券简称", "行业名称"],
    columns="年份",
    values="营业收入_亿元",
)
revenue_wide_ex5["收入增长率_2018_2020"] = revenue_wide_ex5[2020] / revenue_wide_ex5[2018] - 1

revenue_wide_ex5.sort_values("收入增长率_2018_2020", ascending=False).head(10)"""
    ),
    md(
        """## 练习 5.2：构造公司综合摘要表
...
基于 `analysis_ex4` 构造 `company_profile_summary`，每家公司一行，至少包含：`证券代码`、`证券简称`、`行业名称`、`省份`、`2018` 年营业收入、`2020` 年营业收入、`收入增长率_2018_2020`、`2020` 年资产负债率。按 `收入增长率_2018_2020` 从高到低排序，显示前 10 行。"""
    ...
    code(
        """revenue_summary = analysis_ex4.pivot_table(
...
debt_2020 = analysis_ex4.loc[
    analysis_ex4["年份"] == 2020,
    ["证券代码", "资产负债率"],
...
print("公司数量：", analysis_ex4["证券代码"].nunique())
...
构造 2021 年 1 月到 2021 年 6 月的工作日日期。使用随机数生成一列日收益率 `return`，从初始价格 100 出发构造价格 `price`。把结果赋值给以日期为索引的变量 `price_df`，并新增 `lag_price`、`diff`、`pct_return` 三列。显示前 8 行。"""
    ...
    code(
        """dates_ex6 = pd.date_range("2021-01-01", "2021-06-30", freq="B")
rng = np.random.default_rng(2021)

price_df = pd.DataFrame(
    {"return": rng.normal(0.0004, 0.012, len(dates_ex6))},
    index=dates_ex6,
)
...
month_end_price_ex6 = price_df["price"].resample("ME").last()
month_end_price_ex6"""
    ),
    md(
        """## 练习 6.3：从月末价格计算月收益
...
基于 `month_end_price_ex6` 计算月收益率，赋值给变量 `month_return_ex6`。找出月收益率最高的月份和最低的月份，并打印对应月份和收益率。"""
    ...
    code(
        """month_return_ex6 = month_end_price_ex6.pct_change().dropna()

best_month = month_return_ex6.idxmax()
worst_month = month_return_ex6.idxmin()

print("月收益率最高：", best_month.strftime("%Y-%m"), month_return_ex6.loc[best_month])
print("月收益率最低：", worst_month.strftime("%Y-%m"), month_return_ex6.loc[worst_month])
