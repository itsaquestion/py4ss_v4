...
如果行索引看起来刚好也是 0、1、2、3，`.loc[0:3]` 和 `.iloc[:4]` 的结果可能看起来一样。但它们的含义不同：`.loc` 看标签，`.iloc` 看位置。索引被筛选、排序或改成公司代码之后，这个差别会变得很明显。"""
    ),
    code(
...
2. `&` / `|` / `~`：分别表示“并且”“或者”“取反”。复合条件中，每个条件外面都要加括号。忘记括号时，pandas 的报错通常不直观，先回头检查每个条件是不是都包在括号里。
...
选出需要的数据后，很多分析还要把原始字段转换成更接近问题的变量。比如收入、成本和利润可以说明经营规模和结果，但要比较盈利能力，还需要计算净利率；要比较人员效率，可以计算人均营业收入；要观察上市时间，也可以从日期中提取年份并计算上市年限。新增变量后要重点看被改过的列，不需要每次都展示全部列。
...
列运算完成后，先看几列核心输入和新列是否对得上。比如净利率应当和净利润、营业收入方向一致；上市年限不应该是负数。"""
    ),
    code(
        """# 先从日期列生成年度变量。
...
3. `df[条件]["列"] = 新值`：这类连续使用 `[]` 的写法称为链式赋值。pandas 不一定能判断你想修改原表还是临时结果，因此容易触发 warning；有时 warning 后原表并没有被修改。看到这类 warning 时，先把写法改成 `.loc`，再重新检查被修改的列。
...
rank_display[["股票简称", "行业名称", "营业收入", "净利润", "净利率"]].head()  # 查看展示表"""
    ...
    md(
        """阶段 3 小结：我们得到了 `annual_2020_rank`。这一阶段带出了日期列提取、列运算、新增列、按条件赋值、`np.where()`、`pd.cut()`、排序、排名和简单统计。这张排名表先放在这里；接下来换到另一张指标表，处理它里面更接近真实数据的格式问题，等清洗完成后再和经营表合并。
...
真实数据进入统计和合并前，通常要先检查会影响结果的问题。比如证券代码被读成数字会丢掉前导 0，日期列里混入非法日期会影响年份提取，财务指标里出现 `--`、百分号、逗号或“缺失”会影响计算。清洗的目标是让关键列能用于计算、筛选和合并。

范例：读入一张待清洗的公司年度财务指标表，把证券代码整理成 6 位字符串，把统计截止日期整理成日期，把收益率、每股收益和利润指标整理成数值列。处理完成后，这张表要能按“公司 + 日期”和经营表合并。"""
    ),
    md(
        """**读入脏数据，并先看一眼**
...
3. `dtypes`：查看每列类型。数字列如果显示为 `object`，常常说明里面混入了文本。看到这种情况，先找出哪些值不像数字，再处理类型。"""
    ),
    code(
...
3. `duplicated(subset=[...])`：按指定列检查重复。公司年度数据通常要检查“公司 + 日期”是否重复。只看重复数量还不够，最好把重复行展示出来，确认重复是否真的应当删除。"""
    ),
    code(
    ...
    code(
        """# 把重复的公司日期记录展示出来，确认问题长什么样。
dup_mask = indicators_raw.duplicated(subset=["证券代码", "统计截止日期"], keep=False)  # 标记所有重复行
indicators_raw.loc[dup_mask, ["证券代码", "股票简称", "统计截止日期"]].sort_values(["证券代码", "统计截止日期"])  # 查看重复记录"""
    ),
    md(
        """**清理编号和日期**
...
3. `pd.to_datetime(errors="coerce")`：把日期列转换成日期类型；无法识别的日期会变成缺失值。转换后如果缺失值增加，通常说明原始日期里有无法识别的写法。
...
indicators[["证券代码", "股票简称", "统计截止日期", "年份"]].head(8)  # 查看日期处理结果"""
    ...
    code(
        """indicators.loc[indicators["统计截止日期"].isna(), ["证券代码", "股票简称", "年份"]]  # 查看日期转换失败的记录"""
    ),
    md(
        """**清理数值列中的特殊文本**
...
4. `pd.to_numeric(errors="coerce")`：把清理后的文本转成数值；仍无法转换的内容会变成缺失值。这个参数很适合排查脏值：先转换，再回头看哪些原始值变成了缺失。"""
    ),
    code(
        """# 先示范清理一列：加权平均净资产收益率。
...
pd.DataFrame({
    ...
    code(
        """# 找出原来不是缺失、转换后变成缺失的记录。
roe_failed = roe_before.notna() & indicators["加权平均净资产收益率"].isna()  # 标记转换失败
pd.DataFrame({
    "证券代码": indicators.loc[roe_failed, "证券代码"],
    "股票简称": indicators.loc[roe_failed, "股票简称"],
    "原始取值": roe_before.loc[roe_failed],
})  # 查看无法转换的原始值"""
    ),
    code(
        """# 再用同样规则批量清理其他数值列。
...
3. `fillna()`：用指定值填补缺失值。是否填补要看变量含义；这里用每股收益的中位数做演示。连接键缺失通常不适合填补，指标缺失才可能根据业务含义选择填补。"""
    ),
    code(
        """# 删除重复记录和关键连接键缺失的记录。
indicators_clean = (
...
筛选出一部分数据再修改时，显式使用 `.copy()` 生成工作副本。这样更容易判断后续操作影响的是哪一张表。连续使用 `df[条件]["列"] = 新值` 这类链式赋值，pandas 往往会给出 warning，也可能没有改到原表。"""
    ...
    md(
        """阶段 4 小结：我们得到了 `indicators_clean`。这一阶段带出了 `converters`、`isna()`、`duplicated()`、`drop_duplicates()`、`str.strip()`、`zfill()`、`pd.to_datetime()`、`pd.to_numeric()`、`replace()`、`dropna()`、`fillna()`、工作副本和链式赋值 warning。清洗后要看三件事：连接键是否可用，数值列是否已经是数值类型，行数变化是否能解释。
...
很多分析表不是一次拿到的，而是由多张表拼出来的。同一结构的数据可以上下追加；不同信息的数据需要按连接键合并。阶段 4 已经把财务指标表整理成可合并的形态，这里让它和经营表碰面，说明 `concat()` 和 `merge()` 的基本用法。
...
4. 合并后检查：通常检查合并前后行数、关键列缺失数量，并看几行结果。`left` 合并后行数变多，常见原因是右表连接键重复；`inner` 合并后行数变少，说明有些左表记录在右表中匹配不上。"""
    ),
    code(
        """# 把清洗后的财务指标合并到经营表。
indicator_cols = [
...
analysis_df[["证券代码", "证券简称", "年份", "营业收入_亿元", "基本每股收益", "扣非净利润_亿元"]].head(10)  # 查看合并结果"""
    ...
    code(
        """indicators_clean.duplicated(subset=["证券代码", "统计截止日期"]).sum()  # 合并前检查右表连接键是否重复"""
    ),
    code(
...
merge_how_demo  # 查看不同合并方式的行数"""
    ...
    md(
        """阶段 5 小结：我们得到了 `analysis_df`。这一阶段带出了 `concat()`、`ignore_index`、`merge()`、多列连接键、`how` 和合并检查。合并后的表是后面分组、重塑和时间顺序计算的共同起点；以后遇到合并结果异常，先查连接键和行数。
...
公司年度明细表是一家公司一年一行。很多经管问题关心的不是单家公司，而是行业、年份或组内位置。比如，2020 年哪些行业的收入规模更大，不同行业每年的平均净利率如何，某家公司是否高于本行业平均水平。`analysis_df` 已经把经营数据和指标数据接到一起，可以开始从明细表走向分组结论。

范例：从 `analysis_df` 出发，先看 2020 年各行业收入规模和盈利情况，再把行业平均收入带回公司明细表，判断每家公司是否高于本行业平均水平。"""
    ),
    code(
        """analysis_df[[
...
分组汇总后要看分组数量和样本量。只有一两家公司的行业，均值和最大值可能更像个案，不一定代表行业整体。"""
    ),
    code(
...
一个简单判断标准：如果结果只需要每组一行，用 `agg()`；如果结果还要和原表每一行放在一起继续比较，用 `transform()`。"""
    ),
    code(
...
industry_top_company  # 查看每个行业收入最高的公司"""
    ...
    md(
        """阶段 6 小结：这一阶段带出了 `groupby()`、多指标 `agg()`、自定义聚合、`transform()`、每组取前几名、分组循环和在循环结果中复用 `concat()`。分组结果出来后，既要看排序靠前的组，也要看每组公司数，避免把样本很少的组解读得太重。
...
3. `pivot_table()`：把长表整理成宽表。需要指定行索引、展开成列的变量，以及单元格里的数值。重塑后要检查行数和年份列，确认没有把公司合并错，也没有漏掉年份。"""
    ),
    code(
        """# 先看用于重塑的明细列。
...
print("已保存阶段成果")"""
    ...
    md(
        """阶段 7 小结：这一阶段带出了长表和宽表、`pivot_table()`、列运算、`set_index()`、`reset_index()`、构造 `Series` / `DataFrame`，以及保存 Excel 和 CSV。结果表保存前先看几行，保存后再读回来看一眼，这能发现路径、索引和列名问题。
...
3. 计算后要抽几家公司看一眼，确认不会把不同公司的年份接在一起。面板数据里直接对整列 `shift()` 很容易把上一家公司的最后一年错当成下一家公司的上一年。"""
    ),
    code(
        """# 在每家公司内部计算收入变化。
...
panel_by_date.loc["2020", ["证券代码", "证券简称", "行业名称", "营业收入_亿元", "收入增长率"]].head()  # 取出 2020 年记录"""
    ...
    md(
        """阶段 8 小结：这一阶段带出了按时间排序、公司内部 `shift()`、`diff()`、`pct_change()`、用 `cumprod()` 构造累计指数、日期索引和日期切片。时间顺序计算完成后，先看单家公司，再看全部公司；这个顺序能减少很多面板数据中的错位问题。
...
读取 `company_annual_operations_clean.xlsx`，赋值给变量 `annual_ex1`。显示前 5 行和后 5 行，打印行列数、列名列表和各列数据类型。

**练习 1.2**：查看关键经营变量的分布

在 `annual_ex1` 中选择 `营业收入_亿元`、`净利润_亿元`、`总资产_亿元`、`资产负债率` 四列，赋值给变量 `key_business_ex1`。显示这四列的描述统计，并打印 2020 年样本行数。

### 阶段 2 练习：行列选择与条件筛选

**练习 2.1**：找出低负债且盈利的公司

暂时删除 `证券代码` 列，结果赋值给变量 `annual_no_code_ex2`。筛选 2020 年同时满足以下条件的公司：`净利润_亿元 > 0`、`资产负债率 < 0.6`。结果赋值给变量 `low_debt_profit_2020`，只保留 `股票简称`、`营业收入_亿元`、`净利润_亿元`、`资产负债率` 四列，并按 `资产负债率` 从低到高排序。打印结果行数，并显示前 5 行。

**练习 2.2**：选出收入和资产都较高的公司

在 2020 年公司中，筛选 `营业收入_亿元` 高于当年中位数、并且 `总资产_亿元` 高于当年中位数的公司，赋值给变量 `large_revenue_asset_2020`。显示 `股票简称`、`营业收入_亿元`、`总资产_亿元`、`资产负债率`，并查看前 5 行和后 5 行。

### 阶段 3 练习：变量生成与数据修改

**练习 3.1**：找出收入高但净利率较低的公司

在 `annual_no_code_ex2` 中新增 `净利率 = 净利润_亿元 / 营业收入_亿元`。筛选 2020 年 `营业收入_亿元` 高于当年中位数、同时 `净利率` 低于当年中位数的公司，赋值给变量 `high_revenue_low_margin`。这个结果代表“规模不小、利润率相对偏低”的公司。再为 2020 年公司按营业收入生成 `收入排名`。显示 `股票简称`、`营业收入_亿元`、`净利润_亿元`、`净利率`、`收入排名`，并查看前 5 行和后 5 行。
...
在 `annual_no_code_ex2` 中新增 `负债水平`，先全部赋值为“正常”，再用 `.loc` 把 `资产负债率 >= 0.7` 的行改为“较高”。显示 `股票简称`、`年份`、`资产负债率`、`负债水平` 四列的前 8 行，并统计不同 `负债水平` 的数量。

### 阶段 4 练习：常见数据问题处理

**练习 4.1**：清洗证券代码和日期

重新读取 `financial_indicators_dirty.xlsx`，读取时把 `证券代码` 处理成字符串，赋值给变量 `dirty_indicators_ex4`。去掉 `证券代码` 和 `股票简称` 两端空格，把 `证券代码` 补齐为 6 位。把 `统计截止日期` 转成日期类型，并显示转换失败的记录。

**练习 4.2**：清理财务指标数值列

把 `加权平均净资产收益率`、`扣非净资产收益率`、`基本每股收益`、`非经常性损益_亿元`、`扣非净利润_亿元` 转成数值列。要求先处理百分号、逗号、`--`、`缺失` 和空字符串。显示转换后的数据类型，并展示至少一列清理前和清理后的对比结果。

**练习 4.3**：删除重复记录并生成盈利标签

按 `证券代码` 和 `统计截止日期` 删除重复记录，再删除这两个连接键缺失的记录，结果赋值给变量 `indicators_clean_ex4`。用 `扣非净利润_亿元 > 0` 生成 `扣非盈利`，统计 `扣非盈利` 的数量，并显示清理后每列缺失值数量。

### 阶段 5 练习：表的追加与合并

**练习 5.1**：纵向追加年度经营记录

重新读取 `company_annual_operations_clean.xlsx`，读取时把 `证券代码` 处理成 6 位字符串，并把 `股票简称` 重命名为 `证券简称`。生成 `年份`，把 2018-2019 年记录和 2020 年记录拆成两张表，再用 `concat()` 追加回一张表，赋值给变量 `annual_panel_ex5`。显示追加后的年度记录数。

**练习 5.2**：合并清洗后的指标表

把 `annual_panel_ex5` 和 `indicators_clean_ex4` 按 `证券代码`、`统计截止日期` 左连接，赋值给变量 `analysis_ex5`。检查合并前后行数是否一致，统计 `基本每股收益` 的缺失数量，并显示前 8 行。

**练习 5.3**：比较不同合并方式的行数

分别使用 `how="left"`、`how="inner"`、`how="outer"` 合并 `annual_panel_ex5` 和 `indicators_clean_ex4`，构造一张两列的小表：`how` 和 `行数`。显示这张表，并解释哪一种合并保留了经营表的所有记录。

### 阶段 6 练习：分组汇总与组内比较

**练习 6.1**：行业年度摘要表

把 `analysis_ex5` 复制为 `analysis_ex6`，并生成 `净利率`。按 `行业名称` 和 `年份` 分组，计算 `公司数`、`营业收入合计_亿元`、`平均净利率`、`平均资产负债率`，赋值给变量 `industry_year_summary`。显示前 12 行。
...
基于 `analysis_ex6` 构造 `company_profile_summary`，每家公司一行，至少包含：`证券代码`、`证券简称`、`行业名称`、`所属省份`、`2018` 年营业收入、`2020` 年营业收入、`收入增长率_2018_2020`、`2020` 年资产负债率。按 `收入增长率_2018_2020` 从高到低排序，显示前 10 行。
...
把 `analysis_ex6` 按 `证券代码` 和 `统计截止日期` 排序，赋值给变量 `panel_ex8`。选出任意一家公司，赋值给变量 `one_company_ex8`。计算 `上一年营业收入_亿元`、`收入增加额_亿元`、`收入增长率`，并显示该公司的全部年度记录。

**练习 8.2**：在公司内部计算收入增长率

在 `panel_ex8` 中按 `证券代码` 分组，计算每家公司内部的 `上一年营业收入_亿元` 和 `收入增长率`。筛选 2020 年收入增长率最高的 10 家公司，赋值给变量 `top_growth_2020_ex8`，显示 `证券代码`、`证券简称`、`行业名称`、`营业收入_亿元`、`上一年营业收入_亿元`、`收入增长率`。

**练习 8.3**：用日期索引筛选年度记录

把 `panel_ex8` 的 `统计截止日期` 设为索引并排序，赋值给变量 `panel_by_date_ex8`。用日期切片取出 2020 年记录，赋值给变量 `records_2020_ex8`。显示前 8 行，并统计 2020 年记录数。"""
    ),
]
