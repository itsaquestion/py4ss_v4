...
1. **标准库模块**：Python 自带，例如 `math`、`random`、`datetime`。通常不需要额外安装。
2. **第三方模块**：由其他开发者提供，例如 `numpy`、`pandas`、`matplotlib`。本课程使用 Anaconda，许多常用第三方模块已经安装好。
...
- `encoding="utf-8"` 和 `ensure_ascii=False`
- 当前工作目录与文件路径提醒
...
例如用于数学运算的`math`，这是Python自带的模块，其中有大量数学函数，例如开平方根的`sqrt`。
...
还是引入math中的开平方函数`sqrt`。如果代码中频繁使用这个函数，可以直接导入 `sqrt`，后面就不需要每次都写 `math.sqrt()`。
...
要完成一个特定的项目，你可能会写很多代码。没有必要把所有代码都放在一个`.py`文件里。
...
这2个函数都可以归类为计算函数。我们可以把它们放到一个专门的文件中，例如`my_calc.py`。
...
- 如果已经在同一个 Python 会话中导入过 `my_calc`，再修改 `my_calc.py` 后，重新 `import my_calc` 不一定会加载新内容。初学阶段最简单的做法是重启内核，或者重新运行脚本。
- 若需要在模块内放少量自测代码，可使用：
...
- 如果文件就在旁边但仍然导入失败，检查运行代码时的当前工作目录是否正确。
