...
结构化文本常用格式。`json` 用于字典/列表等对象的序列化；`csv` 常用于表格数据交换。

```{python}
import json, csv, io

# JSON：对象 <-> 字符串
obj = {"name": "Alice", "score": 95, "passed": True}
s = json.dumps(obj, ensure_ascii=False)  # 转为 JSON 字符串
print(s)
obj2 = json.loads(s)                     # 还原为 Python 对象（字典）
print(obj2["name"], obj2["score"])
...
rows = list(reader)
print(rows[0]["name"], rows[1]["score"])  # Alice 88
```

将字典写入本地 JSON 文件 → 读取 → 修改 → 覆盖写回（演示完成后清理文件，避免生成多余文件）：
