...
- IndentationError: 缩进不正确或不一致（例如混用 Tab 与空格）。统一为4个空格，避免在同一文件中混用。
- 无限循环: `while` 条件永远为 True 且没有 `break`。检查循环内是否有能改变条件的语句。
- 边界（off-by-one）错误: `range(start, stop)` 不包含 `stop`，容易少算或多算一次。
- 赋值与比较混淆: 在条件判断中把 `==` 错写成 `=`（语法错误），或将 `is` 误用作数值比较。
- 字典/列表访问越界或不存在: `my_list[idx]` 超出范围抛 `IndexError`；访问不存在的字典键抛 `KeyError`，可用 `in` 检查或 `dict.get(key, default)`。
