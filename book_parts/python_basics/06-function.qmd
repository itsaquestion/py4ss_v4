...
从逻辑上来讲，应该是先有`map/filter`：要把一个函数应用到List中的所有元素/过滤元素，这是个基础性的需求，大多数编程语言都有做这件事的办法。我们一般把这种操作统称为`map/filter`。Python的列表推导式，可以看成是一个Python优化版的`map/filter`。
...
这个函数里面调用了变量`n`，但`n`不在`func()`的函数体内，而在上一层。`func()`函数，会把生成自己的环境中的变量（本例中即`n`），也包括在里面：**闭包是一个附带数据的函数**。
