...
*   **参数的不同用法：** 学习位置参数、关键字参数、默认参数，以及如何处理不定数量的参数 (`*args` 和 `**kwargs`)。
*   **函数中的变量作用域：** 理解局部变量、全局变量以及LEGB规则。
*   **不同数据类型作为参数时的影响：** 了解当可变与不可变类型作为参数传递给函数时会发生什么。
*   **(进阶) 递归的初步认识：** 函数如何调用自身来解决特定问题。
*   **(进阶) 函数式编程初步：** 了解 `map`、`filter`、`lambda` 表达式，以及高阶函数和闭包的概念。
...
1. 定义一个函数，由`def`开头
2. 接着是你给这个函数起的名字`add`
3. 名字后面是小括号，里面包含了这个函数的“参数”。这里是2个参数，命名为`x`和`y`。调用这个函数时，需要给它提供这2个参数。
...
2. `z = x + y`。函数体内的2个参数已经有对应的值，所以可以直接使用`x`和`y`。
...
常见的传参错误包括：少传参数、多传参数，或者给同一个参数同时使用位置传参和关键字传参。

```{python}
#| eval: false
print_info("Alex")                    # TypeError: 缺少 age
print_info("Alex", 20, "Shanghai")    # TypeError: 多传了一个参数
print_info("Alex", name="Bob", age=20) # TypeError: name 得到了两个值
```
...
1. 把a_list作为参数x，传递给函数`do_change(x)`，那么在函数内部，变量x和外部a_list指向的是同一个数据，或者说，x成了a_list的别名。
...
print(a_list)
```
<iframe width="800" height="500" frameborder="0" src="https://pythontutor.com/iframe-embed.html#code=a_list%20%3D%20list%28'apple'%29%0A%0Adef%20do_change%28x%29%3A%0A%20%20x%5B0%5D%20%3D%20'B'%0A%20%20return%20x%0A%0Ab_list%20%3D%20do_change%28a_list%29%0A%0Aprint%28b_list%29%0Aprint%28a_list%29&codeDivHeight=400&codeDivWidth=350&cumulative=false&curInstr=0&heapPrimitives=nevernest&origin=opt-frontend.js&py=3&rawInputLstJSON=%5B%5D&textReferences=false"> </iframe>


同样，如果你的设计意图并非“原地修改”，原变量必须保持不变，则可以把x拷贝一份。
...
这里的 `copy()` 是浅拷贝。如果列表或字典内部还嵌套了其他可变对象，内部对象仍可能被共享。嵌套结构需要单独处理，或者使用 `copy.deepcopy()`。
...
func()
```
<span style="color:red"><strong>注意：</strong></span> 函数內部和外部变量重名的情况，要额外小心。因此第二个例子可能是你“忘记了在函数内部定义 a”，而不是你想要“引用全局变量 a”，但是因为有一个全局的变量 a，所以忘记定义 `a` 这个问题可能会被隐藏。
...
下面介绍几个和函数有关的常用工具。这些工具通常归在**函数式编程 (Functional Programming)** 这一类内容中。
...
*   **`map()` 和 `filter()`**：对序列中的元素逐个处理或筛选。
*   **匿名函数 `lambda`**：定义简单的匿名函数。
*   **高阶函数的概念**：函数可以作为参数，也可以作为返回值。
*   **(选学) `reduce()`**：对序列进行累积计算。
...
1. 列表`a`首先经过filter函数的加工，然后再经过map函数的加工，最后得到result。
2. 数据可以依次经过筛选、转换等步骤，最后得到处理后的结果。
...
2. 列表更加符合Python的“风格”
3. 数据量很大时，`map`/`filter` 可能更合适：它们是 lazy（惰性）的，只有在你读取其中的值时，才会把函数**真正**应用上去。
...
从逻辑上来讲，先看`map/filter`也有好处：把一个函数应用到List中的所有元素，或者按照条件过滤元素，是很常见的需求。Python 的列表推导式提供了更简洁的写法。
...
在使用`map`、`filter`，或者列表推导式时，如果要做的操作很短且含义清楚，并且可以写成一行（比如乘以二，或者取最后一个字母），可以直接使用匿名函数。
...
2. 但是，这些函数只有1个地方不同，可以把不同的指数作为参数。


这个函数可以根据参数`n`，返回另一个函数：用于计算n次方的函数。
...
`make_power_function`是一个函数，这个函数会返回另一个函数`func`
...
这个代码里，调用`make_power_function()`函数，会返回另一个函数`func()`。
 
这个函数里面调用了变量`n`，但`n`不在`func()`的函数体内，而在上一层。`func()`会保留它引用的外层变量。本例中，`func()`保留了`n`。
...
`square`函数相当于一个保留了`n=2`的`func()`函数。

1. `make_power_function(2)`，其中`n = 2`
2. 在`make_power_function()`函数的内部，`func(x)`是可以看见这个`n=2`的
3. `return func`的时候，会把`n=2`，一并返回到外层
4. `square = make_power_function(2) `,此时我们把`square`这个名字，绑定给`func`这个函数（闭包），而后者同时还附带了`n=2`
...
提示：
- `np.vectorize` 只是语法糖，不会带来显著加速。
- `DataFrame.apply(axis=1, ...)` 逐行运算通常较慢，应优先列运算或矢量化方案。
