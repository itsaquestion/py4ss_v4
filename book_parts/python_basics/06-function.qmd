...
1. 实现一个函数`my_count()`，计算并返回一个列表的元素数量，但不要使用`len()`函数。应该可以得到：`my_count([2,3,4,5]) == 4`

2. 实现一个函数`my_max_len()`，找到一个字符串列表中最长的字符串，但不要使用`max()`函数。应该可以得到`my_max_len(['a','bb','ccc']) == 'ccc'`。
